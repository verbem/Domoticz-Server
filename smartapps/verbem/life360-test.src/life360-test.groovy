...
definition(
    ...
    oauth: [displayName: "Life360Test", displayLink: "Life360Test"],
    singleInstance: true
) {
	appSetting "clientId"
...
def listUsers () {
    ...
    dynamicPage(name: "listUsersPage", title: "Life360 Users", nextPage: null, uninstall: uninstallOption, install:true) {
        ...
        section("Play user information on:") {
        	input "players", "capability.musicPlayer", multiple: true, required:false, title:"Music Players: "
        }
    }
}
...
def installed() {
    ...
    settings.users.each {memberId->
        ...
        if (member) {
       		...
       		def childDevice = addChildDevice("verbem", "Life360 User", "${app.id}.${member.id}",null,[name:member.firstName, completedSetup: true])
...
def refreshMembers(userId) {
    ...
    state.members = members
    def member = state.members.find{it.id==userId}
    if (member) {
    	return [lon: member.location.longitude, lat: member.location.latitude]
    }
    else {
...
def placeEventHandler() {
    ...
    log.debug presenceState

    if (presenceState) {
        eventText = "${member.firstName} is aangekomen op ${place.name}"
    }
    else {
        eventText = "${member.firstName} is vertrokken van ${place.name}"
    }

    log.debug "Life360 event ${place.name} raised on user: ${member.firstName} "
    deviceWrapper.sendEvent(name:"place", value: place.name)

    if (players) {
        players.each {
            log.debug eventText
            log.debug it.currentValue("level")
            state.sound = textToSpeech(eventText, "Lotte")
            it.playTrackAndRestore(state.sound.uri, state.sound.duration, it.currentValue("level"))
        }    
    }
